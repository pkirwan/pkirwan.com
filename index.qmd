---
title: ""
page-layout: full
toc: false
listing:
  id: blog-listing
  feed: false
  contents: blog
  sort: "date asc"
  type: grid
  grid-columns: 3
  image-height: 200px
  page-size: 10
  categories: false
  fields: [image, reading-time, title, description, categories]
---

## About {#about}

::: {.about-container}
::: {.about-text}
I'm a postdoctoral research fellow at the [Barcelona Institute for Global Health](https://www.isglobal.org), having completed my first postdoc at the [MRC Biostatistics Unit](https://www.mrc-bsu.cam.ac.uk) in Cambridge. My current research focuses on the development of statistical microsimulation and forecasting models to project the health effects of changes in Official Development Assistance (ODA) in low- and middle-income countries. My other research interests include the development of statistical models to estimate infectious disease burden, specifically for HIV.

[GitHub](https://github.com/pkirwan) · [ORCiD](https://orcid.org/0000-0001-6904-0500) · [Mastodon](https://fediscience.org/@pkirwan)
:::

::: {.profile-image}
![](./common/profile-photo.jpg){width=200}
:::
:::

## Statistical methods for survival data {#blog}

This series of blog posts provides an introduction to statistical methods for survival data, with applications for infectious disease modelling and epidemiology. Topics include: classical survival methods, competing risks analysis, and causal inference from observational data.

::: {#blog-listing}
:::

## Software {#software}

::: {.about-container}

::: {.profile-image}
[![](./common/hex_sticker.png){width=150}](https://pkirwan.github.io/cd4backcalc/)
:::

::: {.about-text}
### [cd4backcalc](https://pkirwan.github.io/cd4backcalc/)

cd4backcalc is an R package for Bayesian, discrete-time, multi-state modelling to estimate HIV incidence and undiagnosed HIV prevalence from diagnosis data. The package implements CD4-staged back-calculation models in Stan via `cmdstanr`, with tools to fit models, inspect diagnostics, and plot estimates.

[Documentation](https://pkirwan.github.io/cd4backcalc/) · [Source code](https://github.com/pkirwan/cd4backcalc)
:::

:::

## Recent publications {#recent-publications}

::: {.publications}

::: {.publication}
**2026**

Impact of a revised consensus definition of late HIV diagnosis in Europe: A multi-country study. *Eurosurveillance* (forthcoming).
:::

::: {.publication}
**2024**

[Protection of vaccine boosters and prior infection against mild/asymptomatic and moderate COVID-19 infection in the UK SIREN healthcare worker cohort: October 2023 to March 2024](https://doi.org/10.1016/j.jinf.2024.106293). *Journal of Infection*.
:::

::: {.publication}
**2024**

[Effect of second booster vaccinations and prior infection against SARS-CoV-2 in the UK SIREN healthcare worker cohort](https://doi.org/10.1016/j.lanepe.2023.100809). *The Lancet Regional Health - Europe*.
:::

::: {.publication}
**2022**

[Re-assessing the late HIV diagnosis surveillance definition in the era of increased and frequent testing](https://doi.org/10.1111/hiv.13394). *HIV Medicine*.
:::

::: {.publication}
**2022**

[Trends in COVID-19 hospital outcomes in England before and after vaccine introduction, a cohort study](https://doi.org/10.1038/s41467-022-32458-y). *Nature Communications*.
:::

:::

[Full list of publications](https://scholar.google.co.uk/citations?user=pxDDqncAAAAJ) (via Google Scholar).

## Recorded talks {#talks}

::: {.publication}
**2024**

[Multi-state modelling to estimate infectious disease burden](https://www.youtube.com/watch?v=361Cmi9Zb7I). *University of Cambridge, UK*.
:::

## Contact {#contact}

::: {.contact-container}
::: {.contact-info}
Feel free to reach out about research collaborations, speaking opportunities, or general enquiries:

📧 peter [dot] kirwan [at] isglobal [dot] org

🐘 [@pkirwan@fediscience.org](https://fediscience.org/@pkirwan)
:::
:::
