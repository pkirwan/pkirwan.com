...
image: "./multi-state-models-banner.png"
categories:
...
A stochastic process is formally defined as a collection of random variables $\{X(t),\; t \in T\}$, indexed by a parameter such as time, which varies in a mathematical index set $T$. The variable $X(t)$ is the state of the process at time $t$, and the set $\mathcal{S}$ of all possible values of $X(t)$ is termed the *state space*.
...
![Example of a three-state model with transitions between states according to transition intensities $\{q_{r,s} : r,s \in (1,2,3)\}$](./multi-state-model.png){#fig-msm width=60%}
...
The time spent in a state prior to transition is termed the *sojourn time*. In the case of a time-homogeneous Markov model, the mean sojourn time is the inverse of the transition intensity for remaining in the state:
...
::: {.post-navigation}
[← Previous post](/blog/competing-risks-methods/index.qmd)

[Next post →](/blog/statistical-inference-msms/index.qmd)
:::