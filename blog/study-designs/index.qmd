...
image: "./study-designs-banner.png"
categories:
  - Bias
  - Confounding
execute:
...
![A hierarchy of different types of study designs and the perceived quality of the evidence. Figure from: Bodicoat D.](./study-designs.png){#fig-study-designs}
...
### R example {#r-example-two}

The same $2 \times 2$ table can therefore support different summaries depending on the study design. The code below shows how to compute the prevalence of the outcome and exposure, the odds ratio, and the relative risk from a $2 \times 2$ table in R.

```{r}
#| label: lst-two-by-two-measures
#| lst-cap: "Computing common epidemiological measures from a 2 x 2 table in R."
#| echo: true
...
A <- study_table["Yes", "Yes"]
B <- study_table["Yes", "No"]
C <- study_table["No", "Yes"]
D <- study_table["No", "No"]

study_table
round(
  c(
    prevalence_outcome = (A + C) / (A + B + C + D),
...
The same four cell counts therefore answer different questions: prevalence in cross-sectional studies, odds ratios in case-control studies, and risks or risk ratios in cohort studies.
...
Confounding is related to collider bias, but occurs by not conditioning on an explanatory variable. Confounding introduces a "backdoor path", an additional source of association between the explanatory variable and the outcome, so the measured association cannot be interpreted as a causal effect.
...
### R example {#r-example-epidemic-phase-bias}

The simple sensitivity analysis below shows the basic mechanism in a growing epidemic where severe outcomes are associated with faster symptom onset.

```{r}
#| label: lst-epidemic-phase-bias
#| lst-cap: "Sensitivity analysis for epidemic phase bias in R."
#| echo: true

set.seed(2026)

n             <- 1000
# Growing epidemic: infections concentrated on later days
infection_day <- sample(1:30, n, replace = TRUE, prob = 1:30)
severe        <- rbinom(n, 1, 0.15)
# Severe cases present 3 days faster than mild cases
onset_day     <- infection_day + ifelse(severe == 1, 4, 7)

# Sensitivity analysis: shift the severe-case onset date by c days
cutoff <- 30
epb_df <- data.frame(
  shift_days     = 0:3,
  estimated_risk = round(sapply(0:3, function(c) {
    mean(severe[onset_day + c * severe <= cutoff])
  }), 3),
  true_risk = round(mean(severe), 3)
)

epb_df

ggplot(epb_df, aes(x = shift_days, y = estimated_risk)) +
  geom_line(color = "#E74C3C", linewidth = 1.1) +
  geom_point(color = "#E74C3C", size = 3) +
  geom_hline(
    yintercept = epb_df$true_risk[1],
    linetype = "dashed",
    color = "gray40"
  ) +
  annotate(
    "text",
    x = 0, y = epb_df$true_risk[1],
    label = "True risk", vjust = -0.5, hjust = 0, size = 3.5
  ) +
  scale_x_continuous(name = "Shift (days)", breaks = 0:3) +
  scale_y_continuous(name = "Estimated severe risk") +
  theme_minimal()
```

When no shift is applied, the estimated risk of severe outcomes is biased downwards due to the faster symptom onset among severe cases. As the shift increases, the estimated risk approaches the true risk.
...
::: {.post-navigation}
[← Previous post](/blog/causal-inference/index.qmd)
:::