...
image: "./survival-analysis-banner.jpg"
categories:
  - Censoring
  - Truncation
execute:
...
ggplot() +
  ...
  annotate(
    ...
    x = 11,
    y = 2,
    label = "Left-censoring (initial event time unobserved)",
    hjust = 0,
    size = 3.5
  ) +
  annotate(
...
With all three functions defined, we can visualize how $F(t)$, $S(t)$, and $h(t)$ relate for a simple exponential model with a constant rate $\lambda = 0.15$:

```{r}
#| label: lst-survival-functions
#| lst-cap: "Relating the cumulative incidence, survivor, and hazard functions in R."
#| echo: true

rate      <- 0.15
time_grid <- c(0, 2, 4, 6, 8, 10, 12)

data <- data.frame(
  time = time_grid,
  F_t  = round(pexp(time_grid, rate = rate), 2),
  S_t  = round(pexp(time_grid, rate = rate, lower.tail = FALSE), 2),
  h_t  = rate
)
...
ggplot(data) +
  aes(x = time) +
  geom_line(aes(y = F_t, color = "Cumulative Incidence"), linewidth = 1.2) +
  geom_line(aes(y = S_t, color = "Survivor"), linewidth = 1.2) +
  geom_line(aes(y = h_t, color = "Hazard"), linewidth = 1.2, linetype = "dashed") +
  scale_color_manual(
    name = "Function",
    values = c(
      "Cumulative Incidence" = "#E74C3C",
      "Survivor" = "#3498DB",
      "Hazard" = "#2ECC71"
    )
  ) +
...
Here, the constant hazard demonstrates the "memoryless" property of the exponential model since the instantaneous risk does not depend on how long the individual has already survived.
...
::: {.post-navigation}
[← Previous post](/blog/key-concepts-infectious-disease/index.qmd)

[Next post →](/blog/classical-survival-methods/index.qmd)
:::