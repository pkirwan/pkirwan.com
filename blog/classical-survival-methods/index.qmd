...
image: "./survival-methods-banner.png"
categories:
  - Likelihood
  - Survival models
---
...
Statistical inference is the process of using data to infer the properties of an underlying probability distribution. In the context of survival analysis, we want to estimate the distribution of time-to-event data, which is often incomplete due to censoring or truncation. This is a key challenge in survival analysis, as we need to account for the possibility that we do not observe the event of interest for all individuals in our study. For more details about censored and truncated data see the [previous post](/blog/survival-analysis/index.qmd) in this series.
...
Constructing likelihoods for censored or truncated data requires careful consideration of the information provided by each observation. The [previous post](/blog/survival-analysis/index.qmd) in this series provided a definition of censoring and truncation, and the right and left censoring and truncation times.
...
![Example of a Kaplan-Meier survival plot where time is on the x-axis and the estimated survival probability is on the y-axis.](./survival-methods-banner.png){#km-figure width=80%}

### R example {#r-example-km}

For a short example of the product-limit construction we can use the `survival` package in R to compute a Kaplan-Meier estimate from a small dataset of 10 observations, with some censored and some observed event times:

```{r}
#| label: lst-km-risk-set
#| lst-cap: "Computing a Kaplan-Meier estimate in R."
#| echo: true
...
data.frame(
  ...
  survival = round(km_fit$surv, 2)
)

km_df <- data.frame(
  time  = c(0, km_fit$time),
  surv  = c(1, km_fit$surv),
  event = c(0L, km_fit$n.event)
)

ggplot(km_df, aes(x = time, y = surv)) +
  geom_step(color = "#E74C3C", linewidth = 1.1) +
  geom_point(
    data = subset(km_df, event == 0 & time > 0),
    shape = 3, size = 3, color = "#E74C3C"
  ) +
  scale_y_continuous(limits = c(0, 1), name = "Estimated survival") +
  scale_x_continuous(name = "Follow-up time") +
  theme_minimal()
```

The figure shows the Kaplan-Meier survival curve, with steps at each observed event time and tick marks indicating censored observations. Survival only steps down at observed event times (rows where `n_event > 0`), while censored observations reduce the risk set without causing a jump.
...
In practice, the proportional hazards assumption should be assessed before interpreting hazard ratios, for example using Schoenfeld residuals or by allowing covariate effects to vary over time.

### R example {#r-example-cox}

To illustrate fitting a Cox proportional hazards model, we can use the `survival` package in R to fit a model to the `lung` dataset, which contains survival data for patients with advanced lung cancer. We'll model the hazard of death as a function of age and sex, and check the proportional hazards assumption using Schoenfeld residuals.

```{r}
#| label: lst-cox-model
#| lst-cap: "Fitting and checking a Cox proportional hazards model in R."
#| echo: true

library(survival)

# lung: time to death (days) for advanced lung cancer patients
# status: 1 = censored, 2 = dead; sex: 1 = male, 2 = female
cox_fit <- coxph(Surv(time, status == 2) ~ age + sex, data = lung)

round(summary(cox_fit)$coefficients[, c("coef", "exp(coef)", "Pr(>|z|)")], 3)
round(cox.zph(cox_fit)$table[, "p"], 3)
...
ggplot(coef_df, aes(x = hr, y = term)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "gray50") +
  geom_errorbarh(aes(xmin = lo, xmax = hi), height = 0.1, color = "#2C3E50") +
...
Older age increases the hazard of death (`exp(coef) > 1`), although the effect is not statistically significant (p = 0.06). Female sex is protective (`exp(coef) < 1`), and this effect is statistically significant (p < 0.05). The Schoenfeld test p-values give a quick check of the proportional hazards assumption (satisfied when p > 0.05).
...
::: {.post-navigation}
[← Previous post](/blog/survival-analysis/index.qmd)

[Next post →](/blog/competing-risks-methods/index.qmd)
:::